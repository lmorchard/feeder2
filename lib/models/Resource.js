...
module.exports = function (models, baseClass, baseProto) {
  ...
  _.extend(Resource, baseClass, {
    ...
    pollAll: function (db, options, each, done) {
      ...
      options.request = request.defaults({
        agent: new http.Agent({
          maxSockets: options.concurrency
        })
      });

      var cursor = db.collection(Resource.collection).find();

      var exhausted = false;
      var enqueueing = false;

      var queue = async.queue(function (resource, next) {
      ...
      var enqueue = function () {
        ...
        if (exhausted || enqueueing) { return; }
        enqueueing = true;
